...
Operações para listagem financiamentos

All URIs are relative to http://api.banco.com.br/open-banking/opendata-financings/v1, except if the operation defines another base path.
...
```php
getBusinessFinancings($page, $page_size): \OpenAPI\Client\Model\ResponseBusinessFinancings
```
...
```php
getPersonalFinancings($page, $page_size): \OpenAPI\Client\Model\ResponsePersonalFinancings
```
...
try {
    $result = $apiInstance->getPersonalFinancings($page, $page_size);
    print_r($result);
} catch (Exception $e) {
