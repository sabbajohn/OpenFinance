...
| ------------- | ------------- | ------------- |
| [**creditFixedIncomesGetInvestmentsInvestmentIdTransactions()**](TransactionsApi.md#creditFixedIncomesGetInvestmentsInvestmentIdTransactions) | **GET** /investments/{investmentId}/transactions | Obtém as movimentações históricas (últimos 12 meses) da operação de Renda Fixa Crédito identificada por investmentId. |


## `creditFixedIncomesGetInvestmentsInvestmentIdTransactions()`

```php
creditFixedIncomesGetInvestmentsInvestmentIdTransactions($investment_id, $authorization, $x_fapi_interaction_id, $x_fapi_auth_date, $x_fapi_customer_ip_address, $x_customer_user_agent, $page, $page_size, $pagination_key, $from_transaction_date, $to_transaction_date): \OpenAPI\Client\Model\ResponseCreditFixedIncomesTransactions
```
...
$from_transaction_date = Fri Dec 31 21:00:00 BRT 2021; // \DateTime | Data inicial de filtragem.       [Restrição] Deve obrigatoriamente ser enviado caso o campo toTransactionDate seja informado. Caso não seja informado, deve ser assumido o dia atual.
$to_transaction_date = Fri Dec 30 21:00:00 BRT 2022; // \DateTime | Data final de filtragem.       [Restrição] Deve obrigatoriamente ser enviado caso o campo fromTransactionDate seja informado. Caso não seja informado, deve ser assumido o dia atual.

try {
    $result = $apiInstance->creditFixedIncomesGetInvestmentsInvestmentIdTransactions($investment_id, $authorization, $x_fapi_interaction_id, $x_fapi_auth_date, $x_fapi_customer_ip_address, $x_customer_user_agent, $page, $page_size, $pagination_key, $from_transaction_date, $to_transaction_date);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling TransactionsApi->creditFixedIncomesGetInvestmentsInvestmentIdTransactions: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\ResponseCreditFixedIncomesTransactions**](../Model/ResponseCreditFixedIncomesTransactions.md)
