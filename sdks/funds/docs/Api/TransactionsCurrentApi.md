...
| ------------- | ------------- | ------------- |
| [**fundsGetInvestmentsInvestmentIdTransactionsCurrent()**](TransactionsCurrentApi.md#fundsGetInvestmentsInvestmentIdTransactionsCurrent) | **GET** /investments/{investmentId}/transactions-current | Obtém as movimentações recentes da operação de Fundos de Investimento identificada por investmentId. O período a ser considerado para apresentação de movimentações será de até 7 dias - 7 dias anteriores da consulta, incluindo o dia da consulta (D-6). |


## `fundsGetInvestmentsInvestmentIdTransactionsCurrent()`

```php
fundsGetInvestmentsInvestmentIdTransactionsCurrent($investment_id, $authorization, $x_fapi_interaction_id, $x_fapi_auth_date, $x_fapi_customer_ip_address, $x_customer_user_agent, $page, $page_size, $pagination_key, $from_transaction_conversion_date, $to_transaction_conversion_date): \OpenAPI\Client\Model\ResponseFundsTransactionsCurrent
```
...
$from_transaction_conversion_date = Thu May 20 21:00:00 BRT 2021; // \DateTime | Data inicial de filtragem. O período máximo utilizado no filtro é de 7 dias inclusive (D-6).  [Restrição] Deve obrigatoriamente ser enviado caso o campo toTransactionConversionDate seja informado. Caso não seja informado, deve ser assumido o dia atual.
$to_transaction_conversion_date = Thu May 20 21:00:00 BRT 2021; // \DateTime | Data final de filtragem. O período máximo utilizado no filtro é de 7 dias inclusive (D-6).  [Restrição] Deve obrigatoriamente ser enviado caso o campo fromTransactionConversionDate seja informado. Caso não seja informado, deve ser assumido o dia atual.

try {
    $result = $apiInstance->fundsGetInvestmentsInvestmentIdTransactionsCurrent($investment_id, $authorization, $x_fapi_interaction_id, $x_fapi_auth_date, $x_fapi_customer_ip_address, $x_customer_user_agent, $page, $page_size, $pagination_key, $from_transaction_conversion_date, $to_transaction_conversion_date);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling TransactionsCurrentApi->fundsGetInvestmentsInvestmentIdTransactionsCurrent: ', $e->getMessage(), PHP_EOL;
}
```
...
| **from_transaction_conversion_date** | **\DateTime**| Data inicial de filtragem. O período máximo utilizado no filtro é de 7 dias inclusive (D-6).  [Restrição] Deve obrigatoriamente ser enviado caso o campo toTransactionConversionDate seja informado. Caso não seja informado, deve ser assumido o dia atual. | [optional] |
| **to_transaction_conversion_date** | **\DateTime**| Data final de filtragem. O período máximo utilizado no filtro é de 7 dias inclusive (D-6).  [Restrição] Deve obrigatoriamente ser enviado caso o campo fromTransactionConversionDate seja informado. Caso não seja informado, deve ser assumido o dia atual. | [optional] |

### Return type

[**\OpenAPI\Client\Model\ResponseFundsTransactionsCurrent**](../Model/ResponseFundsTransactionsCurrent.md)
