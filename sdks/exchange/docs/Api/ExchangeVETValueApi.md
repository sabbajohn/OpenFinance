# OpenAPI\Client\ExchangeVETValueApi
...
| ------------- | ------------- | ------------- |
| [**exchangeGetVetValue()**](ExchangeVETValueApi.md#exchangeGetVetValue) | **GET** /vet-values | Conjunto de informações de Câmbio para valor do VET. |


## `exchangeGetVetValue()`

```php
exchangeGetVetValue($page, $page_size): \OpenAPI\Client\Model\OKResponseExchangeVetValue
```
...
$apiInstance = new OpenAPI\Client\Api\ExchangeVETValueApi(
...
try {
    $result = $apiInstance->exchangeGetVetValue($page, $page_size);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ExchangeVETValueApi->exchangeGetVetValue: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\OKResponseExchangeVetValue**](../Model/OKResponseExchangeVetValue.md)
