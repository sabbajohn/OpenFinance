...
**applications** | [**\OpenAPI\Client\Model\ApplicationRate[]**](ApplicationRate.md) | Lista  das faixas de cobrança da taxa efetiva aplicada pela contratação de crédito |
**minimum_rate** | **string** | Percentual mínimo cobrado (taxa efetiva) no mês de referência, para o Empréstimo contratado  A apuração pode acontecer com até 4 casas decimais. O preenchimento deve respeitar as 4 casas decimais, mesmo que venham preenchidas com zeros (representação de porcentagem p.ex: 0.1500. Este valor representa 15%. O valor 1 representa 100%) |
