...
| ------------- | ------------- | ------------- |
| [**fundsGetInvestmentsInvestmentIdTransactions()**](TransactionsApi.md#fundsGetInvestmentsInvestmentIdTransactions) | **GET** /investments/{investmentId}/transactions | Obtém as movimentações históricas (últimos 12 meses) da operação de Fundos de Investimento identificada por investmentId. |


## `fundsGetInvestmentsInvestmentIdTransactions()`

```php
fundsGetInvestmentsInvestmentIdTransactions($investment_id, $authorization, $x_fapi_interaction_id, $x_fapi_auth_date, $x_fapi_customer_ip_address, $x_customer_user_agent, $page, $page_size, $pagination_key, $from_transaction_conversion_date, $to_transaction_conversion_date): \OpenAPI\Client\Model\ResponseFundsTransactions
```
...
$from_transaction_conversion_date = Thu May 20 21:00:00 BRT 2021; // \DateTime | Data inicial de filtragem.  [Restrição] Deve obrigatoriamente ser enviado caso o campo toTransactionConversionDate seja informado. Caso não seja informado, deve ser assumido o dia atual.
$to_transaction_conversion_date = Thu May 20 21:00:00 BRT 2021; // \DateTime | Data final de filtragem.  [Restrição] Deve obrigatoriamente ser enviado caso o campo fromTransactionConversionDate seja informado. Caso não seja informado, deve ser assumido o dia atual.

try {
    $result = $apiInstance->fundsGetInvestmentsInvestmentIdTransactions($investment_id, $authorization, $x_fapi_interaction_id, $x_fapi_auth_date, $x_fapi_customer_ip_address, $x_customer_user_agent, $page, $page_size, $pagination_key, $from_transaction_conversion_date, $to_transaction_conversion_date);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling TransactionsApi->fundsGetInvestmentsInvestmentIdTransactions: ', $e->getMessage(), PHP_EOL;
}
```
...
| **from_transaction_conversion_date** | **\DateTime**| Data inicial de filtragem.  [Restrição] Deve obrigatoriamente ser enviado caso o campo toTransactionConversionDate seja informado. Caso não seja informado, deve ser assumido o dia atual. | [optional] |
| **to_transaction_conversion_date** | **\DateTime**| Data final de filtragem.  [Restrição] Deve obrigatoriamente ser enviado caso o campo fromTransactionConversionDate seja informado. Caso não seja informado, deve ser assumido o dia atual. | [optional] |

### Return type

[**\OpenAPI\Client\Model\ResponseFundsTransactions**](../Model/ResponseFundsTransactions.md)
