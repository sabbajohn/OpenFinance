# OpenAPI\Client\ConcurrencyManagementApi
...
Informa se o contrato está disponível para solicitação de portabilidade de crédito.
...
$apiInstance = new OpenAPI\Client\Api\ConcurrencyManagementApi(
...
try {
    $result = $apiInstance->creditPortabilityGetCreditOperationsContratIdPortabilityEligibility($contract_id, $authorization, $x_fapi_interaction_id, $x_fapi_auth_date, $x_fapi_customer_ip_address, $x_customer_user_agent);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ConcurrencyManagementApi->creditPortabilityGetCreditOperationsContratIdPortabilityEligibility: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\ResponsePortabilityEligibility**](../Model/ResponsePortabilityEligibility.md)
