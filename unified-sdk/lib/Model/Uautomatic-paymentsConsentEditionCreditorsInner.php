...
/**
 * ConsentEditionCreditorsInner Class Doc Comment
 *
 * @category Class
 ...
 */
class Uautomatic-paymentsConsentEditionCreditorsInner implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ConsentEdition_creditors_inner';
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['name']) && !preg_match("/^([A-Za-zÀ-ÖØ-öø-ÿ,.@:&*+_<>()!?\/\\\\$%\\d' -]+)$/", $this->container['name'])) {
     ...
     * Sets name
     *
     * @param string|null $name Em caso de pessoa natural deve ser informado o nome completo do titular da conta do recebedor.   Em caso de pessoa jurídica deve ser informada a razão social ou o nome fantasia da conta do recebedor.
     *
     * @return self
     */
    public function setName($name)
    {
        ...
        if ((mb_strlen($name) > 120)) {
            throw new \InvalidArgumentException('invalid length for $name when calling ConsentEditionCreditorsInner., must be smaller than or equal to 120.');
        }
        if ((!preg_match("/^([A-Za-zÀ-ÖØ-öø-ÿ,.@:&*+_<>()!?\/\\\\$%\\d' -]+)$/", ObjectSerializer::toString($name)))) {
            throw new \InvalidArgumentException("invalid value for \$name when calling ConsentEditionCreditorsInner., must conform to the pattern /^([A-Za-zÀ-ÖØ-öø-ÿ,.@:&*+_<>()!?\/\\\\$%\\d' -]+)$/.");
        }
