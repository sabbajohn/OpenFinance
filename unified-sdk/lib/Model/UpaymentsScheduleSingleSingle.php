...
/**
 * ScheduleSingleSingle Class Doc Comment
 *
 * @category Class
 ...
 */
class UpaymentsScheduleSingleSingle implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ScheduleSingle_single';
    ...
    */
    ...
    public function setDate($date)
    {
        ...
        if ((mb_strlen($date) > 10)) {
            throw new \InvalidArgumentException('invalid length for $date when calling ScheduleSingleSingle., must be smaller than or equal to 10.');
        }
        if ((!preg_match("/^(\\d{4})-(1[0-2]|0?[1-9])-(3[01]|[12][0-9]|0?[1-9])$/", ObjectSerializer::toString($date)))) {
            throw new \InvalidArgumentException("invalid value for \$date when calling ScheduleSingleSingle., must conform to the pattern /^(\\d{4})-(1[0-2]|0?[1-9])-(3[01]|[12][0-9]|0?[1-9])$/.");
        }
