...
/**
 * FinancingsChargeOverParcel Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class UfinancingsFinancingsChargeOverParcel implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'FinancingsChargeOverParcel';
    ...
    */
     ...
     * Sets charge_amount
     *
     * @param float $charge_amount Valor do pagamento do encargo pago fora da parcela. Expresso em valor monetário com no mínimo 2 casas e no máximo 4 casas decimais.
     *
     * @return self
     */
    public function setChargeAmount($charge_amount)
    {
        ...
        if ((mb_strlen($charge_amount) > 21)) {
            throw new \InvalidArgumentException('invalid length for $charge_amount when calling FinancingsChargeOverParcel., must be smaller than or equal to 21.');
        }
        if ((mb_strlen($charge_amount) < 4)) {
            throw new \InvalidArgumentException('invalid length for $charge_amount when calling FinancingsChargeOverParcel., must be bigger than or equal to 4.');
        }
        if ((!preg_match("/^-?\\d{1,15}\\.\\d{2,4}$/", ObjectSerializer::toString($charge_amount)))) {
            throw new \InvalidArgumentException("invalid value for \$charge_amount when calling FinancingsChargeOverParcel., must conform to the pattern /^-?\\d{1,15}\\.\\d{2,4}$/.");
        }
    ...
    public function setChargeAdditionalInfo($charge_additional_info)
    {
        ...
        if ((mb_strlen($charge_additional_info) > 140)) {
            throw new \InvalidArgumentException('invalid length for $charge_additional_info when calling FinancingsChargeOverParcel., must be smaller than or equal to 140.');
        }
        if ((!preg_match("/^[^\\s](.*[^\\s])?$/", ObjectSerializer::toString($charge_additional_info)))) {
            throw new \InvalidArgumentException("invalid value for \$charge_additional_info when calling FinancingsChargeOverParcel., must conform to the pattern /^[^\\s](.*[^\\s])?$/.");
        }
