...
/**
 * 422ResponseErrorCreateConsentErrorsInner Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class Uconsents422ResponseErrorCreateConsentErrorsInner implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = '_422ResponseErrorCreateConsent_errors_inner';
    ...
    public const CODE_DEPENDE_MULTIPLA_ALCADA = 'DEPENDE_MULTIPLA_ALCADA';
    public const CODE_ESTADO_CONSENTIMENTO_INVALIDO = 'ESTADO_CONSENTIMENTO_INVALIDO';
    ...
    {
        return [
            self::CODE_DEPENDE_MULTIPLA_ALCADA,
            self::CODE_ESTADO_CONSENTIMENTO_INVALIDO,
    ...
    */
     ...
     * Sets title
     *
     * @param string $title Título específico do erro reportado, de acordo com o código enviado: - DEPENDE_MULTIPLA_ALCADA: Necessário aprovação de múltipla alçada. - ESTADO_CONSENTIMENTO_INVALIDO: Estado inválido do consentimento. - DATA_EXPIRACAO_INVALIDA: Nova data para expiração do consentimento é inválida. - ERRO_NAO_MAPEADO: Utilizado quando não houver um code de erro definido. O texto deve deixar claro o motivo do erro ocorrido.
     *
     * @return self
     */
    public function setTitle($title)
    {
        ...
        if ((mb_strlen($title) > 255)) {
            throw new \InvalidArgumentException('invalid length for $title when calling 422ResponseErrorCreateConsentErrorsInner., must be smaller than or equal to 255.');
        }
        if ((!preg_match("/[\\w\\W\\s]*/", ObjectSerializer::toString($title)))) {
            throw new \InvalidArgumentException("invalid value for \$title when calling 422ResponseErrorCreateConsentErrorsInner., must conform to the pattern /[\\w\\W\\s]*/.");
        }
     ...
     * Sets detail
     *
     * @param string $detail Título específico do erro reportado, de acordo com o código enviado: - DEPENDE_MULTIPLA_ALCADA: O consentimento informado não pode ser renovado sem redirecionamento porque depende de múltipla alçada para aprovação. - ESTADO_CONSENTIMENTO_INVALIDO: O consentimento informado não pode ser renovado sem redirecionamento porque está em um estado que não permite a renovação. - DATA_EXPIRACAO_INVALIDA: O consentimento informado não pode ser renovado pois a nova data de expiração não segue a convenção do ecossistema. - ERRO_NAO_MAPEADO: Utilizado quando não houver um code de erro definido. O texto deve deixar claro o motivo do erro ocorrido.
     *
     * @return self
     */
    public function setDetail($detail)
    {
        ...
        if ((mb_strlen($detail) > 2048)) {
            throw new \InvalidArgumentException('invalid length for $detail when calling 422ResponseErrorCreateConsentErrorsInner., must be smaller than or equal to 2048.');
        }
        if ((!preg_match("/[\\w\\W\\s]*/", ObjectSerializer::toString($detail)))) {
            throw new \InvalidArgumentException("invalid value for \$detail when calling 422ResponseErrorCreateConsentErrorsInner., must conform to the pattern /[\\w\\W\\s]*/.");
        }
