...
/**
 * UnarrangedAccountOverdraftFinanceCharge Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class Uunarranged-accounts-overdraftUnarrangedAccountOverdraftFinanceCharge implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'UnarrangedAccountOverdraftFinanceCharge';
    ...
    */
    ...
    public function setChargeAdditionalInfo($charge_additional_info)
    {
        ...
        if ((!preg_match("/^[^\\s](.*[^\\s])?$/", ObjectSerializer::toString($charge_additional_info)))) {
            throw new \InvalidArgumentException("invalid value for \$charge_additional_info when calling UnarrangedAccountOverdraftFinanceCharge., must conform to the pattern /^[^\\s](.*[^\\s])?$/.");
        }
    ...
    public function setChargeRate($charge_rate)
    {
        ...
        if ((mb_strlen($charge_rate) > 8)) {
            throw new \InvalidArgumentException('invalid length for $charge_rate when calling UnarrangedAccountOverdraftFinanceCharge., must be smaller than or equal to 8.');
        }
        if ((mb_strlen($charge_rate) < 8)) {
            throw new \InvalidArgumentException('invalid length for $charge_rate when calling UnarrangedAccountOverdraftFinanceCharge., must be bigger than or equal to 8.');
        }
        if ((!preg_match("/^\\d{1}\\.\\d{6}$/", ObjectSerializer::toString($charge_rate)))) {
            throw new \InvalidArgumentException("invalid value for \$charge_rate when calling UnarrangedAccountOverdraftFinanceCharge., must conform to the pattern /^\\d{1}\\.\\d{6}$/.");
        }
