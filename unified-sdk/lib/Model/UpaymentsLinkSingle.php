 ...
 */
class UpaymentsLinkSingle implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'LinkSingle';
    ...
    */
    ...
    public function setSelf($self)
    {
        ...
        if ((mb_strlen($self) > 2000)) {
            throw new \InvalidArgumentException('invalid length for $self when calling LinkSingle., must be smaller than or equal to 2000.');
        }
        if ((!preg_match("/^(https:\/\/)?(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{2,256}\\.[a-z]{2,6}\\b([-a-zA-Z0-9@:%_\\+.~#?&\/\/=]*)$/", ObjectSerializer::toString($self)))) {
            throw new \InvalidArgumentException("invalid value for \$self when calling LinkSingle., must conform to the pattern /^(https:\/\/)?(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{2,256}\\.[a-z]{2,6}\\b([-a-zA-Z0-9@:%_\\+.~#?&\/\/=]*)$/.");
        }
