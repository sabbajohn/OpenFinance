...
/**
 * ResponsePortabilitiesByPortabilityIdDataRejectionReason Class Doc Comment
 *
 * @category Class
 ...
 */
class Ucredit-portabilityResponsePortabilitiesByPortabilityIdDataRejectionReason implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ResponsePortabilitiesByPortabilityId_data_rejection_reason';
    ...
    public const TYPE_CANCELADO_PELO_CLIENTE = 'CANCELADO_PELO_CLIENTE';
    public const TYPE_SALDO_DEVEDOR_ATUALIZADO_SUBSTANCIALMENTE_DIVERGENTE = 'SALDO_DEVEDOR_ATUALIZADO_SUBSTANCIALMENTE_DIVERGENTE';
    public const TYPE_POLITICA_DE_CREDITO = 'POLITICA_DE_CREDITO';
    public const TYPE_RETENCAO_DO_CLIENTE = 'RETENCAO_DO_CLIENTE';
    public const TYPE_CONTRATO_JA_LIQUIDADO = 'CONTRATO_JA_LIQUIDADO';
    public const TYPE_DIVERGENCIA_DE_PAGAMENTO_EFETUADO = 'DIVERGENCIA_DE_PAGAMENTO_EFETUADO';
    public const TYPE_DECURSO_DO_PRAZO_PARA_PAGAMENTO = 'DECURSO_DO_PRAZO_PARA_PAGAMENTO';
    public const TYPE_PORTABILIDADE_CANCELADA_POR_FALTA_DE_LIQUIDACAO = 'PORTABILIDADE_CANCELADA_POR_FALTA_DE_LIQUIDACAO';
    public const TYPE_PORTABILIDADE_EM_ANDAMENTO = 'PORTABILIDADE_EM_ANDAMENTO';
    public const TYPE_CLIENTE_COM_ACAO_JUDICIAL = 'CLIENTE_COM_ACAO_JUDICIAL';
    public const TYPE_MODALIDADE_DA_OPERACAO_INCOMPATIVEL = 'MODALIDADE_DA_OPERACAO_INCOMPATIVEL';
    ...
    {
        return [
            self::TYPE_CANCELADO_PELO_CLIENTE,
            self::TYPE_SALDO_DEVEDOR_ATUALIZADO_SUBSTANCIALMENTE_DIVERGENTE,
            self::TYPE_POLITICA_DE_CREDITO,
            self::TYPE_RETENCAO_DO_CLIENTE,
            self::TYPE_CONTRATO_JA_LIQUIDADO,
            self::TYPE_DIVERGENCIA_DE_PAGAMENTO_EFETUADO,
            self::TYPE_DECURSO_DO_PRAZO_PARA_PAGAMENTO,
            self::TYPE_PORTABILIDADE_CANCELADA_POR_FALTA_DE_LIQUIDACAO,
            self::TYPE_PORTABILIDADE_EM_ANDAMENTO,
            self::TYPE_CLIENTE_COM_ACAO_JUDICIAL,
            self::TYPE_MODALIDADE_DA_OPERACAO_INCOMPATIVEL,
            self::TYPE_OUTROS,
        ];
    }
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['type_additional_info']) && (mb_strlen($this->container['type_additional_info']) > 144)) {
            $invalidProperties[] = "invalid value for 'type_additional_info', the character length must be smaller than or equal to 144.";
        }

        if (!is_null($this->container['type_additional_info']) && !preg_match("/^[^\\s](?:.*[^\\s])?$/", $this->container['type_additional_info'])) {
            $invalidProperties[] = "invalid value for 'type_additional_info', must be conform to the pattern /^[^\\s](?:.*[^\\s])?$/.";
        }
     ...
     *
     * @param string|null $type_additional_info Informação sobre a disponibilidade ou não de um contrato para a portabilidade de crédito.  Ao utilizar essa opção, é fortemente recomendável enviar um ticket como sugestão da estrutura Open Finance para discussão e mapeamento em futuras versões.  [RESTRIÇÃO] Campo de preenchimento obrigatório quando campo type for igual a OUTROS.
     *
     * @return self
     */
    public function setTypeAdditionalInfo($type_additional_info)
    {
        ...
        if ((mb_strlen($type_additional_info) > 144)) {
            throw new \InvalidArgumentException('invalid length for $type_additional_info when calling ResponsePortabilitiesByPortabilityIdDataRejectionReason., must be smaller than or equal to 144.');
        }
        if ((!preg_match("/^[^\\s](?:.*[^\\s])?$/", ObjectSerializer::toString($type_additional_info)))) {
            throw new \InvalidArgumentException("invalid value for \$type_additional_info when calling ResponsePortabilitiesByPortabilityIdDataRejectionReason., must conform to the pattern /^[^\\s](?:.*[^\\s])?$/.");
        }
