...
/**
 * ScheduleCustomCustom Class Doc Comment
 *
 * @category Class
 * @description [Restrição] As datas enviadas na lista de datas (array “dates”) não podem ser repetidas.  Caso datas repetidas sejam enviadas, o detentor deve rejeitar a criação do consentimento, informando o erro PARAMETRO_INVALIDO.
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 * @implements \ArrayAccess<string, mixed>
 */
class UpaymentsScheduleCustomCustom implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ScheduleCustom_custom';
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if ((count($this->container['dates']) > 60)) {
            $invalidProperties[] = "invalid value for 'dates', number of items must be less than or equal to 60.";
        }

        if ((count($this->container['dates']) < 2)) {
            $invalidProperties[] = "invalid value for 'dates', number of items must be greater than or equal to 2.";
        }

        if ($this->container['additional_information'] === null) {
            $invalidProperties[] = "'additional_information' can't be null";
        }
        if ((mb_strlen($this->container['additional_information']) > 255)) {
            $invalidProperties[] = "invalid value for 'additional_information', the character length must be smaller than or equal to 255.";
        }

        if (!preg_match("/[\\w\\W\\s]*/", $this->container['additional_information'])) {
     ...
     * Sets dates
     *
     * @param \DateTime[] $dates Define os dias em que estão planejadas as ocorrências das liquidações.
     *
     * @return self
     */
    public function setDates($dates)
    {
        ...
        if ((count($dates) > 60)) {
            throw new \InvalidArgumentException('invalid value for $dates when calling ScheduleCustomCustom., number of items must be less than or equal to 60.');
        }
        if ((count($dates) < 2)) {
            throw new \InvalidArgumentException('invalid length for $dates when calling ScheduleCustomCustom., number of items must be greater than or equal to 2.');
        }
     ...
     *
     * @param string $additional_information Texto livre para Iniciador preencher de forma compreensível pelo usuário aprovador/pagador.  O texto pode ser utilizado pelo detentor para exibição do resumo da transação durante aprovação do usuário aprovador/pagador.
     *
     * @return self
     */
    public function setAdditionalInformation($additional_information)
    {
        ...
        if ((mb_strlen($additional_information) > 255)) {
            throw new \InvalidArgumentException('invalid length for $additional_information when calling ScheduleCustomCustom., must be smaller than or equal to 255.');
        }
        if ((!preg_match("/[\\w\\W\\s]*/", ObjectSerializer::toString($additional_information)))) {
            throw new \InvalidArgumentException("invalid value for \$additional_information when calling ScheduleCustomCustom., must conform to the pattern /[\\w\\W\\s]*/.");
        }
