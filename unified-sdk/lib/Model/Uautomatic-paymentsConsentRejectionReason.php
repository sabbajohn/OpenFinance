...
/**
 * ConsentRejectionReason Class Doc Comment
 *
 * @category Class
 * @description Informações sobre o motivo da rejeição
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 * @implements \ArrayAccess<string, mixed>
 */
class Uautomatic-paymentsConsentRejectionReason implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ConsentRejectionReason';
    ...
    public const CODE_FALHA_INFRAESTRUTURA = 'FALHA_INFRAESTRUTURA';
    public const CODE_TEMPO_EXPIRADO_AUTORIZACAO = 'TEMPO_EXPIRADO_AUTORIZACAO';
    public const CODE_REJEITADO_USUARIO = 'REJEITADO_USUARIO';
    public const CODE_CONTAS_ORIGEM_DESTINO_IGUAIS = 'CONTAS_ORIGEM_DESTINO_IGUAIS';
    public const CODE_CONTA_NAO_PERMITE_PAGAMENTO = 'CONTA_NAO_PERMITE_PAGAMENTO';
    public const CODE_SALDO_INSUFICIENTE = 'SALDO_INSUFICIENTE';
    public const CODE_VALOR_ACIMA_LIMITE = 'VALOR_ACIMA_LIMITE';
    public const CODE_AUTENTICACAO_DIVERGENTE = 'AUTENTICACAO_DIVERGENTE';
    ...
    public function getCodeAllowableValues()
    {
        return [
            ...
            self::CODE_FALHA_INFRAESTRUTURA,
            self::CODE_TEMPO_EXPIRADO_AUTORIZACAO,
            self::CODE_REJEITADO_USUARIO,
            self::CODE_CONTAS_ORIGEM_DESTINO_IGUAIS,
            self::CODE_CONTA_NAO_PERMITE_PAGAMENTO,
            self::CODE_SALDO_INSUFICIENTE,
            self::CODE_VALOR_ACIMA_LIMITE,
            self::CODE_AUTENTICACAO_DIVERGENTE,
        ];
    }
    ...
    */
     ...
     * Sets code
     *
     * @param string $code Código indicador do motivo de rejeição. - NAO_INFORMADO - FALHA_INFRAESTRUTURA - TEMPO_EXPIRADO_AUTORIZACAO - REJEITADO_USUARIO - CONTAS_ORIGEM_DESTINO_IGUAIS - CONTA_NAO_PERMITE_PAGAMENTO - SALDO_INSUFICIENTE - VALOR_ACIMA_LIMITE - AUTENTICACAO_DIVERGENTE
     *
     * @return self
     */
     ...
     * Sets detail
     *
     * @param string $detail Detalhe sobre o motivo de rejeição indicado no campo `/data/rejection/reason/code` - NAO_INFORMADO: Não informada pela detentora de conta; - FALHA_INFRAESTRUTURA: [Descrição de qual falha na infraestrutura inviabilizou o processamento]; - TEMPO_EXPIRADO_AUTORIZACAO: Consentimento expirou antes que o usuário pudesse confirmá-lo; - REJEITADO_USUARIO: O usuário rejeitou a autorização do consentimento; - CONTAS_ORIGEM_DESTINO_IGUAIS: A conta selecionada é igual à conta destino e não permite realizar esse pagamento; - CONTA_NAO_PERMITE_PAGAMENTO: A conta selecionada é do tipo [salario/investimento/liquidação/outros] e não permite realizar esse pagamento; - SALDO_INSUFICIENTE: A conta selecionada não possui saldo suficiente para realizar o pagamento; - VALOR_ACIMA_LIMITE: O valor ultrapassa o limite estabelecido para permitir a realização de transações pelo cliente; - AUTENTICACAO_DIVERGENTE : Usuário autenticado no detentor diverge do usuário autenticado no iniciador;
     *
     * @return self
     */
    public function setDetail($detail)
    {
        ...
        if ((mb_strlen($detail) > 2048)) {
            throw new \InvalidArgumentException('invalid length for $detail when calling ConsentRejectionReason., must be smaller than or equal to 2048.');
        }
        if ((!preg_match("/[\\w\\W\\s]*/", ObjectSerializer::toString($detail)))) {
            throw new \InvalidArgumentException("invalid value for \$detail when calling ConsentRejectionReason., must conform to the pattern /[\\w\\W\\s]*/.");
        }
