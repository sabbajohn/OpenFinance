 ...
 */
class UpaymentsIdentification implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'Identification';
    ...
    */
    ...
    public function setCpfCnpj($cpf_cnpj)
    {
        ...
        if ((mb_strlen($cpf_cnpj) > 14)) {
            throw new \InvalidArgumentException('invalid length for $cpf_cnpj when calling Identification., must be smaller than or equal to 14.');
        }
        if ((mb_strlen($cpf_cnpj) < 11)) {
            throw new \InvalidArgumentException('invalid length for $cpf_cnpj when calling Identification., must be bigger than or equal to 11.');
        }
        if ((!preg_match("/^\\d{11}$|^\\d{14}$/", ObjectSerializer::toString($cpf_cnpj)))) {
            throw new \InvalidArgumentException("invalid value for \$cpf_cnpj when calling Identification., must conform to the pattern /^\\d{11}$|^\\d{14}$/.");
        }
    ...
    public function setName($name)
    {
        ...
        if ((mb_strlen($name) > 120)) {
            throw new \InvalidArgumentException('invalid length for $name when calling Identification., must be smaller than or equal to 120.');
        }
        if ((!preg_match("/^([A-Za-zÀ-ÖØ-öø-ÿ,.@:&*+_<>()!?\/\\\\$%\\d' -]+)$/", ObjectSerializer::toString($name)))) {
            throw new \InvalidArgumentException("invalid value for \$name when calling Identification., must conform to the pattern /^([A-Za-zÀ-ÖØ-öø-ÿ,.@:&*+_<>()!?\/\\\\$%\\d' -]+)$/.");
        }
