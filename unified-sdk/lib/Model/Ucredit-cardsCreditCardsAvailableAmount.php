...
/**
 * CreditCardsAvailableAmount Class Doc Comment
 *
 * @category Class
 ...
 */
class Ucredit-cardsCreditCardsAvailableAmount implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'CreditCardsAvailableAmount';
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['amount']) && (mb_strlen($this->container['amount']) < 4)) {
            $invalidProperties[] = "invalid value for 'amount', the character length must be bigger than or equal to 4.";
        }

        if (!is_null($this->container['amount']) && !preg_match("/^-?\\d{1,15}\\.\\d{2,4}$/", $this->container['amount'])) {
     ...
     * Sets amount
     *
     * @param float|null $amount Valor disponível do limite informado expresso em valor monetário com no mínimo 2 casas e no máximo 4 casas decimais.  [Restrição] O campo é obrigatório caso isLimitFlexible for igual a false.
     *
     * @return self
     */
    public function setAmount($amount)
    {
        ...
        if ((mb_strlen($amount) > 21)) {
            throw new \InvalidArgumentException('invalid length for $amount when calling CreditCardsAvailableAmount., must be smaller than or equal to 21.');
        }
        if ((mb_strlen($amount) < 4)) {
            throw new \InvalidArgumentException('invalid length for $amount when calling CreditCardsAvailableAmount., must be bigger than or equal to 4.');
        }
        if ((!preg_match("/^-?\\d{1,15}\\.\\d{2,4}$/", ObjectSerializer::toString($amount)))) {
            throw new \InvalidArgumentException("invalid value for \$amount when calling CreditCardsAvailableAmount., must conform to the pattern /^-?\\d{1,15}\\.\\d{2,4}$/.");
        }
     ...
     * Sets currency
     *
     * @param string|null $currency Moeda referente ao limite informado, segundo modelo ISO-4217. p.ex. 'BRL.' Todos os saldos informados estão representados com a moeda vigente do Brasil.    [Restrição] O campo é obrigatório caso isLimitFlexible for igual a false.
     *
     * @return self
     */
    public function setCurrency($currency)
    {
        ...
        if ((mb_strlen($currency) > 3)) {
            throw new \InvalidArgumentException('invalid length for $currency when calling CreditCardsAvailableAmount., must be smaller than or equal to 3.');
        }
        if ((!preg_match("/^[A-Z]{3}$/", ObjectSerializer::toString($currency)))) {
            throw new \InvalidArgumentException("invalid value for \$currency when calling CreditCardsAvailableAmount., must conform to the pattern /^[A-Z]{3}$/.");
        }
