...
/**
 * VrpVrpGlobalLimits Class Doc Comment
 *
 * @category Class
 ...
 */
class Uautomatic-paymentsVrpVrpGlobalLimits implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'Vrp_vrp_globalLimits';
    ...
    */
     ...
     * Sets quantity_limit
     *
     * @param int|null $quantity_limit Quantidade máxima de ocorrência  de pagamentos, após atingir este valor, o consentimento deve ir para o status \"CONSUMED\"
     *
     * @return self
     */
     ...
     *
     * @param string|null $transaction_limit Valor transacional máximo para pagamentos sob este consentimento, após atingir este valor, o consentimento deve ir para o status \"CONSUMED\".
     *
     * @return self
     */
    public function setTransactionLimit($transaction_limit)
    {
        ...
        if ((mb_strlen($transaction_limit) > 19)) {
            throw new \InvalidArgumentException('invalid length for $transaction_limit when calling VrpVrpGlobalLimits., must be smaller than or equal to 19.');
        }
        if ((mb_strlen($transaction_limit) < 4)) {
            throw new \InvalidArgumentException('invalid length for $transaction_limit when calling VrpVrpGlobalLimits., must be bigger than or equal to 4.');
        }
        if ((!preg_match("/^((\\d{1,16}\\.\\d{2}))$/", ObjectSerializer::toString($transaction_limit)))) {
            throw new \InvalidArgumentException("invalid value for \$transaction_limit when calling VrpVrpGlobalLimits., must conform to the pattern /^((\\d{1,16}\\.\\d{2}))$/.");
        }
