...
/**
 * ConsentsDebtorAccount Class Doc Comment
 *
 * @category Class
 * @description Objeto que contém a identificação da conta de origem do pagador. As informações quanto à conta de origem do pagador poderão ser trazidas no consentimento para a detentora, caso a iniciadora tenha coletado essas informações do cliente. No caso em que o cliente não preenche os dados na iniciadora, a detentora deverá persistir as informações da conta selecionada seguindo as condições abaixo.  [Restrição]  - AUTHORISED e CONSUMED: Para esses dois status, o preenchimento do campo deverá ser obrigatório. - REJECTED: Para este status o preenchimento é condicional, dado que há cenários em que a detentora também não terá conhecimento da conta origem, pois a mesma não foi selecionada pelo usuário. Nos casos em que houver seleção, a conta deve ser preenchida obrigatoriamente.
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 * @implements \ArrayAccess<string, mixed>
 */
class UpaymentsConsentsDebtorAccount implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ConsentsDebtorAccount';
    ...
    */
    ...
    public function setIspb($ispb)
    {
        ...
        if ((mb_strlen($ispb) > 8)) {
            throw new \InvalidArgumentException('invalid length for $ispb when calling ConsentsDebtorAccount., must be smaller than or equal to 8.');
        }
        if ((mb_strlen($ispb) < 8)) {
            throw new \InvalidArgumentException('invalid length for $ispb when calling ConsentsDebtorAccount., must be bigger than or equal to 8.');
        }
        if ((!preg_match("/^[0-9]{8}$/", ObjectSerializer::toString($ispb)))) {
            throw new \InvalidArgumentException("invalid value for \$ispb when calling ConsentsDebtorAccount., must conform to the pattern /^[0-9]{8}$/.");
        }
    ...
    public function setIssuer($issuer)
    {
        ...
        if ((mb_strlen($issuer) > 4)) {
            throw new \InvalidArgumentException('invalid length for $issuer when calling ConsentsDebtorAccount., must be smaller than or equal to 4.');
        }
        if ((mb_strlen($issuer) < 1)) {
            throw new \InvalidArgumentException('invalid length for $issuer when calling ConsentsDebtorAccount., must be bigger than or equal to 1.');
        }
        if ((!preg_match("/^[0-9]{1,4}$/", ObjectSerializer::toString($issuer)))) {
            throw new \InvalidArgumentException("invalid value for \$issuer when calling ConsentsDebtorAccount., must conform to the pattern /^[0-9]{1,4}$/.");
        }
    ...
    public function setNumber($number)
    {
        ...
        if ((mb_strlen($number) > 20)) {
            throw new \InvalidArgumentException('invalid length for $number when calling ConsentsDebtorAccount., must be smaller than or equal to 20.');
        }
        if ((mb_strlen($number) < 1)) {
            throw new \InvalidArgumentException('invalid length for $number when calling ConsentsDebtorAccount., must be bigger than or equal to 1.');
        }
        if ((!preg_match("/^[0-9]{1,20}$/", ObjectSerializer::toString($number)))) {
            throw new \InvalidArgumentException("invalid value for \$number when calling ConsentsDebtorAccount., must conform to the pattern /^[0-9]{1,20}$/.");
        }
