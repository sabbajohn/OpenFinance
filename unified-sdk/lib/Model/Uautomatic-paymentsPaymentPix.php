 ...
 */
class Uautomatic-paymentsPaymentPix implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'PaymentPix';
    ...
    */
    ...
    public function setAmount($amount)
    {
        ...
        if ((mb_strlen($amount) > 19)) {
            throw new \InvalidArgumentException('invalid length for $amount when calling PaymentPix., must be smaller than or equal to 19.');
        }
        if ((mb_strlen($amount) < 4)) {
            throw new \InvalidArgumentException('invalid length for $amount when calling PaymentPix., must be bigger than or equal to 4.');
        }
        if ((!preg_match("/^((\\d{1,16}\\.\\d{2}))$/", ObjectSerializer::toString($amount)))) {
            throw new \InvalidArgumentException("invalid value for \$amount when calling PaymentPix., must conform to the pattern /^((\\d{1,16}\\.\\d{2}))$/.");
        }
    ...
    public function setCurrency($currency)
    {
        ...
        if ((mb_strlen($currency) > 3)) {
            throw new \InvalidArgumentException('invalid length for $currency when calling PaymentPix., must be smaller than or equal to 3.');
        }
        if ((!preg_match("/^([A-Z]{3})$/", ObjectSerializer::toString($currency)))) {
            throw new \InvalidArgumentException("invalid value for \$currency when calling PaymentPix., must conform to the pattern /^([A-Z]{3})$/.");
        }
