<?php
/**
 * ResponsePostRecurringConsentDataRevocation
 *
 * PHP version 8.1
 *
 * @category Class
...
/**
 * ResponsePostRecurringConsentDataRevocation Class Doc Comment
 *
 * @category Class
 * @description Objeto contendo as informações de revogação dos consentimentos.  [Restrição] Campo de preenchimento obrigatório caso status do consentimento igual a \&quot;REVOKED\&quot;.
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 * @implements \ArrayAccess<string, mixed>
 */
class Uautomatic-paymentsResponsePostRecurringConsentDataRevocation implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ResponsePostRecurringConsent_data_revocation';
    ...
    protected static $openAPITypes = [
        ...
        'revoked_from' => 'string',
        'revoked_at' => '\DateTime',
        'reason' => '\OpenAPI\Client\Model\ResponsePostRecurringConsentDataRevocationReason'
    ];
    ...
    public const REVOKED_BY_INICIADORA = 'INICIADORA';
    public const REVOKED_BY_USUARIO = 'USUARIO';
    public const REVOKED_BY_DETENTORA = 'DETENTORA';
    public const REVOKED_FROM_INICIADORA = 'INICIADORA';
    public const REVOKED_FROM_DETENTORA = 'DETENTORA';
     ...
     * @return string[]
     */
    public function getRevokedByAllowableValues()
    {
        return [
            self::REVOKED_BY_INICIADORA,
            self::REVOKED_BY_USUARIO,
            self::REVOKED_BY_DETENTORA,
        ];
    }
     ...
     * @return string[]
     */
    public function getRevokedFromAllowableValues()
    {
        return [
            self::REVOKED_FROM_INICIADORA,
            self::REVOKED_FROM_DETENTORA,
        ];
    }
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('revoked_from', $data ?? [], null);
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if ($this->container['revoked_by'] === null) {
            $invalidProperties[] = "'revoked_by' can't be null";
        }
        $allowedValues = $this->getRevokedByAllowableValues();
        if (!is_null($this->container['revoked_by']) && !in_array($this->container['revoked_by'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'revoked_by', must be one of '%s'",
                $this->container['revoked_by'],
                implode("', '", $allowedValues)
            );
        }

        if ($this->container['revoked_from'] === null) {
            $invalidProperties[] = "'revoked_from' can't be null";
        }
        $allowedValues = $this->getRevokedFromAllowableValues();
        if (!is_null($this->container['revoked_from']) && !in_array($this->container['revoked_from'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'revoked_from', must be one of '%s'",
                $this->container['revoked_from'],
                implode("', '", $allowedValues)
            );
        }

        if ($this->container['revoked_at'] === null) {
            $invalidProperties[] = "'revoked_at' can't be null";
        }
        if ((mb_strlen($this->container['revoked_at']) > 20)) {
            $invalidProperties[] = "invalid value for 'revoked_at', the character length must be smaller than or equal to 20.";
        }

        if (!preg_match("/^(\\d{4})-(1[0-2]|0?[1-9])-(3[01]|[12][0-9]|0?[1-9])T(?:[01]\\d|2[0123]):(?:[012345]\\d):(?:[012345]\\d)Z$/", $this->container['revoked_at'])) {
            $invalidProperties[] = "invalid value for 'revoked_at', must be conform to the pattern /^(\\d{4})-(1[0-2]|0?[1-9])-(3[01]|[12][0-9]|0?[1-9])T(?:[01]\\d|2[0123]):(?:[012345]\\d):(?:[012345]\\d)Z$/.";
        }
     ...
     * Sets revoked_by
     *
     * @param string $revoked_by Quem iniciou a solicitação de revogação   - INICIADORA - USUARIO - DETENTORA
     *
     * @return self
     */
    public function setRevokedBy($revoked_by)
    {
        if (is_null($revoked_by)) {
            throw new \InvalidArgumentException('non-nullable revoked_by cannot be null');
        }
        $allowedValues = $this->getRevokedByAllowableValues();
        if (!in_array($revoked_by, $allowedValues, true)) {
     ...
     * @return string
     */
    public function getRevokedFrom()
    {
        return $this->container['revoked_from'];
    }

    /**
     * Sets revoked_from
     *
     * @param string $revoked_from Canal onde iniciou-se o processo de revogação    - INICIADORA   - DETENTORA
     *
     * @return self
     */
    public function setRevokedFrom($revoked_from)
    {
        if (is_null($revoked_from)) {
            throw new \InvalidArgumentException('non-nullable revoked_from cannot be null');
        }
        $allowedValues = $this->getRevokedFromAllowableValues();
        if (!in_array($revoked_from, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'revoked_from', must be one of '%s'",
                    $revoked_from,
        ...
        $this->container['revoked_from'] = $revoked_from;
     ...
     * Sets revoked_at
     *
     * @param \DateTime $revoked_at Data e hora em que o consentimento foi revogado
     *
     * @return self
     */
    public function setRevokedAt($revoked_at)
    {
        ...
        if ((mb_strlen($revoked_at) > 20)) {
            throw new \InvalidArgumentException('invalid length for $revoked_at when calling ResponsePostRecurringConsentDataRevocation., must be smaller than or equal to 20.');
        }
        if ((!preg_match("/^(\\d{4})-(1[0-2]|0?[1-9])-(3[01]|[12][0-9]|0?[1-9])T(?:[01]\\d|2[0123]):(?:[012345]\\d):(?:[012345]\\d)Z$/", ObjectSerializer::toString($revoked_at)))) {
            throw new \InvalidArgumentException("invalid value for \$revoked_at when calling ResponsePostRecurringConsentDataRevocation., must conform to the pattern /^(\\d{4})-(1[0-2]|0?[1-9])-(3[01]|[12][0-9]|0?[1-9])T(?:[01]\\d|2[0123]):(?:[012345]\\d):(?:[012345]\\d)Z$/.");
        }
     ...
     * Gets reason
     *
     * @return \OpenAPI\Client\Model\ResponsePostRecurringConsentDataRevocationReason|null
     */
     ...
     * Sets reason
     *
     * @param \OpenAPI\Client\Model\ResponsePostRecurringConsentDataRevocationReason|null $reason reason
     *
     * @return self
     */
