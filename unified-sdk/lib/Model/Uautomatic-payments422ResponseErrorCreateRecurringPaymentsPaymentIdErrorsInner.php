<?php
/**
 * 422ResponseErrorCreateRecurringPaymentsPaymentIdErrorsInner
 *
 * PHP version 8.1
 *
 * @category Class
...
/**
 * 422ResponseErrorCreateRecurringPaymentsPaymentIdErrorsInner Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class Uautomatic-payments422ResponseErrorCreateRecurringPaymentsPaymentIdErrorsInner implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = '_422ResponseErrorCreateRecurringPaymentsPaymentId_errors_inner';
    ...
    public const CODE_PAGAMENTO_NAO_PERMITE_CANCELAMENTO = 'PAGAMENTO_NAO_PERMITE_CANCELAMENTO';
    public const CODE_CANCELAMENTO_FORA_PERIODO_PERMITIDO = 'CANCELAMENTO_FORA_PERIODO_PERMITIDO';
    ...
    {
        return [
            self::CODE_PAGAMENTO_NAO_PERMITE_CANCELAMENTO,
            self::CODE_CANCELAMENTO_FORA_PERIODO_PERMITIDO,
    ...
    */
     ...
     * Sets code
     *
     * @param string $code - PAGAMENTO_NAO_PERMITE_CANCELAMENTO: Pagamento está com um status que não permite cancelamento. - CANCELAMENTO_FORA_PERIODO_PERMITIDO: O usuário solicitou o cancelamento fora da janela de tempo permitido. - PARAMETRO_INVALIDO: Os parâmetros informados não obedecem a formatação especificada. - PARAMETRO_NAO_INFORMADO: Algum ou todos os campos obrigatórios não foram informados.
     *
     * @return self
     */
     ...
     * Sets title
     *
     * @param string $title - PAGAMENTO_NAO_PERMITE_CANCELAMENTO: Pagamento está com um status que não permite cancelamento. - CANCELAMENTO_FORA_PERIODO_PERMITIDO: O usuário solicitou o cancelamento fora da janela de tempo permitido. - PARAMETRO_INVALIDO: Os parâmetros informados não obedecem a formatação especificada. - PARAMETRO_NAO_INFORMADO: Algum ou todos os campos obrigatórios não foram informados.
     *
     * @return self
     */
    public function setTitle($title)
    {
        ...
        if ((mb_strlen($title) > 255)) {
            throw new \InvalidArgumentException('invalid length for $title when calling 422ResponseErrorCreateRecurringPaymentsPaymentIdErrorsInner., must be smaller than or equal to 255.');
        }
        if ((!preg_match("/[\\w\\W\\s]*/", ObjectSerializer::toString($title)))) {
            throw new \InvalidArgumentException("invalid value for \$title when calling 422ResponseErrorCreateRecurringPaymentsPaymentIdErrorsInner., must conform to the pattern /[\\w\\W\\s]*/.");
        }
     ...
     * Sets detail
     *
     * @param string $detail - PAGAMENTO_NAO_PERMITE_CANCELAMENTO: Pagamento está com um status que não permite cancelamento. - CANCELAMENTO_FORA_PERIODO_PERMITIDO: O usuário solicitou o cancelamento fora da janela de tempo permitido. - PARAMETRO_INVALIDO: Os parâmetros informados não obedecem a formatação especificada. - PARAMETRO_NAO_INFORMADO: Algum ou todos os campos obrigatórios não foram informados.
     *
     * @return self
     */
    public function setDetail($detail)
    {
        ...
        if ((mb_strlen($detail) > 2048)) {
            throw new \InvalidArgumentException('invalid length for $detail when calling 422ResponseErrorCreateRecurringPaymentsPaymentIdErrorsInner., must be smaller than or equal to 2048.');
        }
        if ((!preg_match("/[\\w\\W\\s]*/", ObjectSerializer::toString($detail)))) {
            throw new \InvalidArgumentException("invalid value for \$detail when calling 422ResponseErrorCreateRecurringPaymentsPaymentIdErrorsInner., must conform to the pattern /[\\w\\W\\s]*/.");
        }
