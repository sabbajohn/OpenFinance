...
/**
 * PatchPixPaymentCancellation Class Doc Comment
 *
 * @category Class
 ...
 */
class UpaymentsPatchPixPaymentCancellation implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'PatchPixPaymentCancellation';
    ...
    */
    ...
    public function setCancelledAt($cancelled_at)
    {
        ...
        if ((mb_strlen($cancelled_at) > 20)) {
            throw new \InvalidArgumentException('invalid length for $cancelled_at when calling PatchPixPaymentCancellation., must be smaller than or equal to 20.');
        }
        if ((!preg_match("/^(\\d{4})-(1[0-2]|0?[1-9])-(3[01]|[12][0-9]|0?[1-9])T(?:[01]\\d|2[0123]):(?:[012345]\\d):(?:[012345]\\d)Z$/", ObjectSerializer::toString($cancelled_at)))) {
            throw new \InvalidArgumentException("invalid value for \$cancelled_at when calling PatchPixPaymentCancellation., must conform to the pattern /^(\\d{4})-(1[0-2]|0?[1-9])-(3[01]|[12][0-9]|0?[1-9])T(?:[01]\\d|2[0123]):(?:[012345]\\d):(?:[012345]\\d)Z$/.");
        }
